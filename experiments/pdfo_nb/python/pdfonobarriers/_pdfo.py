...
def pdfonobarriers(fun, x0, args=(), method=None, bounds=None, constraints=(), options=None):
    ...
    args: tuple, optional
        The extra-arguments to pass to the objective function. For example,
            ...
            ``pdfonobarriers(fun, x0, args, ...)``

        is equivalent to

            ``pdfonobarriers(lambda x: fun(x, *args), x0, ...)``
    ...
    >>> options = {'maxfev': 50}
    >>> pdfonobarriers(np.cos, -1, constraints=lin_con, options=options)
    ...
    >>> options = {'maxfev': 200}
    >>> pdfonobarriers(obj, [0, 1], bounds=bounds, constraints=[lin_con, nonlin_con], options=options)
    ...
    if prob_info['infeasible']:
        ...
        # transfer to the call to postpdfo of pdfonobarriers to appear to the output of pdfonobarriers.

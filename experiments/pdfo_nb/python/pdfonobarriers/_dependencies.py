...
invoker_list.append('pdfonobarriers')
...
def prepdfo(fun, x0, args=(), method=None, bounds=None, constraints=(), options=None):
    ...
    if len(stack()) >= 4 and stack()[2][3].lower() == 'pdfonobarriers':
        # The invoker is a solver called by pdfonobarriers, then prepdfo should have been called in pdfonobarriers. In this case, we set
    ...
    # solver itself.
    if method is None and invoker != 'pdfonobarriers':
        method = invoker
    ...
    if not _prob_solv_match(prob_info['refined_type'], invoker.lower()):
        if invoker.lower() == 'pdfonobarriers':
            raise SystemError(
                '{}: UNEXPECTED ERROR: problem and solver do not match; it should not happen when the invoker is pdfonobarriers '
    ...
    # Select a solver if the invoker is 'pdfonobarriers' and no one is provided.
    if invoker.lower() == 'pdfonobarriers':
...
def _options_validation(invoker, options, method, lenx0, lb, ub, list_warnings):
    ...
    # If method == '' or None, then invoker must be pdfonobarriers, and a solver will be selected later; when the solver is
    ...
    # If prepdfo is called by pdfonobarriers, and if pdfonobarriers has been called without precising the method, npt will be set to np.nan.
...
def _prob_solv_match(problem_type, solver):
    ...
    if solver not in ['bobyqa', 'cobyla', 'lincoa', 'newuoa', 'uobyqa', 'pdfonobarriers']:
...
def _solver_selection(invoker, method, options, prob_info, list_warnings):
    ...
    # possible solvers
    fun_name = stack()[0][3]  # name of the current function
    local_invoker_list = ['pdfonobarriers']
...
def _pre_rhobeg_x0(invoker, x0, lb, ub, user_options_fields, options, list_warnings):
    # possible solvers
    fun_name = stack()[0][3]  # name of the current function
    local_invoker_list = ['pdfonobarriers', 'bobyqa']
...
def postpdfo(x, fx, exitflag, output, method, nf, fhist, options, prob_info, constrviolation=0, chist=None):
    ...
    # Validate chist.
    if not (chist is None and method in ['pdfonobarriers', 'newuoa', 'uobyqa']) and \
    ...
    # If the invoker is a solver called by pdfonobarriers, then let pdfonobarriers do the post-processing.
    output['x'] = x_c
    output['fun'] = fx_c
    ...
    # If the solver is not called by pdfonobarriers (can be pdfonobarriers directly), perform the post-processing.

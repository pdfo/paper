...
def cobyla(fun, x0, args=(), bounds=None, constraints=(), options=None):
    ...
    -------
    res: OptimizeResult
        The results of the solver. Check ``pdfonobarriers.OptimizeResult`` for a description of the attributes.
    ...
    pdfonobarriers : Powell's Derivative-Free Optimization solvers

    Examples
    --------
    1. The following code

    >>> from python.pdfonobarriers import *
    >>> import numpy as np
    ...
    # called by pdfonobarriers, then prob_info will not be passed to postpdfo, and hence the warning message will be lost. To the
    ...
    if invoker != 'pdfonobarriers' and prob_info['infeasible']:
    ...
    elif invoker != 'pdfonobarriers' and prob_info['nofreex']:
    ...
    elif invoker != 'pdfonobarriers' and prob_info['feasibility_problem'] and \
        ...
        if min_nw >= max_int:
            executor = invoker.lower() if invoker == 'pdfonobarriers' else fun_name

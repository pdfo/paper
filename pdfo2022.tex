...
according to~\cref{eq:itpls}, a trial point~$\iter^{\trust}$ is then obtained by solving approximately the trust-region subproblem
\begin{equation}
    \label{eq:trsp}
    \begin{split}
...
\Gls{cobyla} builds linear models by the system~\cref{eq:litpls}, with~$\xpt$ being an interpolation
...
\gls{cobyla} maintains the inverse of the coefficient matrix for~\cref{eq:litpls}
and updates it along the iterations.
Since each iteration of \gls{cobyla} alters the interpolation set~$\xpt$ by only one
point~(see \cref{ssec:iptset}), the coefficient matrix is modified by a rank-$1$ update, and hence
its inverse can be updated according to the Sherman-Morrison-Woodbury formula.
\gls{uobyqa} does the same for~\cref{eq:qitpls},
except that~\cite[\S~4]{Powell_2002} describes the update in terms of the Lagrange functions of the
interpolation problem~\cref{eq:qitpls}, the coefficients of a Lagrange function corresponding
...
of~\cref{eq:undqitp}--\cref{eq:leastchange}. The update is also done by the
...
At a normal iteration, a point~$\iter^{\trust} \in \R^n$ is computed by solving the trust-region subproblem~\cref{eq:trsp}, and Powell's \gls{dfo} methods update the interpolation set as
\begin{equation*}
    \xpt[k + 1] = \left(\xpt \cup \set{\iter^{\trust}}\right) \setminus \set{y_k^{\drop}},
\end{equation*}
...
When it comes to solving the trust-region subproblems~\cref{eq:trsp}, \gls{newuoa} employs the

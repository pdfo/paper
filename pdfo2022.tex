...
\begin{abstract}
    Late Professor M. J. D. Powell designed five derivative-free optimization algorithms, namely \gls{cobyla}, \gls{uobyqa}, \gls{newuoa}, \gls{bobyqa}, and \gls{lincoa}.
    The algorithms were implemented in Fortran 77 and therefore may not be easily accessible to some users.
    This paper aims to introduce the PDFO (Powell's Derivative-Free Optimization) package, which is developed to provide both Python and MATLAB interfaces to Powell's code.
    With PDFO, users can call each of Powell's algorithm directly based on their own choice.
    ...
    When a problem can be solved by multiple algorithms (for example, an unconstrained problem can be solved by all the five algorithms), the selection is made based on experimental results on the CUTEst problems.
...
For each supported language, \gls{pdfo} provides a simple subroutine that can invoke one of Powell's solvers according to the user's request (if any) or according to the type of the problem to solve. The current release (version 1.2) of \gls{pdfo} supports Python and MATLAB, with more languages to be covered in the future.
...
\begin{enumerate}
    ...
    For instance, if the problem has linear constraints~$\aeq x = \beq$, \gls{pdfo} can rewrite it into a problem on the null space of~$\aeq$, eliminating such constraints and reducing the dimension.
...
Algorithms with this strategy are referred to as model-based methods, and they often make use of the models within a trust-region framework~\cite{Conn_Gould_Toint_2000,Conn_Scheinberg_Vicente_2009a,Yuan_2015}, but line-search methods also exist~\cite{Berahas_Byrd_Nocedal_2019,Shi_Etal_2022}.
...
At iteration~$k$, the algorithms construct a linear of quadratic trust-region model~$\objm$ for the objective function~$f$ to meet the interpolation condition
\begin{equation}
    \label{eq:itpls}
...
Instead of repeating Powell's description of these algorithms, we provide a sketch of them in the sequel, emphasizing the trust-region subproblem, the interpolation problem, and the interpolation~set.

\subsubsection{The trust-region subproblem}

In all the five algorithms, iteration~$k$ places the trust-region center~$\iter$ at the \enquote{best} point where the objective function and constraints have been evaluated so far.
...
\end{equation}
which is a linear system with~$\obj(y^\base) \in \R$ and~$\nabla \obj(y^\base) \in \R^n$ being the unknowns, the degrees of freedom being~$n+1$.
...
\begin{equation}
    ...
    \end{split}
\end{equation}
where~$\qspace$ is the space of polynomials on~$\R^n$ with the degree being~$2$ or lower, and~$\func_k$ is the aforementioned functional.
...
= \nabla \obj(\iter)$, and~$\nabla Q(\iter[k - 1]) = \obj(\iter[k - 1])$ (see~\cite[Theorem~4.2]{Dennis_Schnabel_1979}), with~$\iter$ and~$\iter[k - 1]$ being the current and the previous iterate respectively.
...
for the problem~\cref{eq:undqitp,eq:leastchange}~\cite{Powell_2004b}.
...
Indeed, the inverse matrices provide the coefficients of the Lagrange functions for the corresponding interpolation problems~\cite{Powell_2001,Powell_2004a}.
...
It should recycle points from previous iterations, at which the objective function has already been evaluated; meanwhile, it needs to maintain the well poisedness~\cite{Sauer_Xu_1995,Conn_Scheinberg_Vicente_2009b} of the interpolation set in order to guarantee the well conditioning of the interpolation problem.\todo{There is something missing below}
\begin{equation*}
...
$\xpt$ always contains the trust-region center~$\iter$.\todo{Geometry steps}

\subsection{\glsfmtshort{cobyla}}
\label{ssec:cobyla}

Powell released the \gls{cobyla} solver in May 1992 and published the \gls{cobyla}
paper\cite{Powell_1994} in \citedate{Powell_1994}.
...
In this context, the interpolation set~$\xpt$ is \alert{poised} if an only if the volume of the~$n$-simplex engendered by~$\xpt$ to be nonzero.
...
In doing so, the method attempts to reduce the~$\ell_\infty$-violation of the constraint models while ensuring that the trial point lies in the trust region.\todo{Verify this paragraph.}
...
To do so, at each iteration, it models the objective function with a quadratic of the form
\begin{equation}
    \label{eq:polm}
    \objm(x) \eqdef \grad^{\T} (x - \iter) + (x - \iter)^{\T} \hess (x - \iter),
\end{equation}
obtained by fully-determined interpolation on the set~$\xpt$ containing~$N = (n + 1)(n + 2) / 2$ points.
...
As we already mentioned, it is essential to maintain some geometrical properties of~$\xpt$ to ensure the existence and the uniqueness of the models from a computational viewpoint.
...
It states that given a point~$\bar{y} \in \xpt$ to remove from the interpolation set, its most suitable substitute to build~$\xpt[k + 1]$ solves the subproblem
\begin{equation}
    \label{eq:biglag}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
where~$\ell_{\bar{y}} : \R^n \to \R$ denotes the Lagrange polynomial associated with~$\bar{y}$, defined by~$\ell_{\bar{y}}(y) = \delta_{\bar{y}, y}$ for each~$y \in \xpt$, where~$\delta_{\bar{y}, y}$ is the Kronecker delta.
...
Given the set~$\xpt$, containing typically~$\mathcal{O}(n)$ points (the default value~$2n + 1$ being recommended), the model~$\objm$ is the quadratic satisfying the interpolation conditions whose Hessian's update is least in Frobenius norm, that is
\begin{equation}
    \label{eq:varmod}
    \begin{aligned}
        \objm \eqdef \argmin_{Q \in \mathcal{P}_{n, 2}} & \quad \norm{\nabla^2 Q - \nabla^2 \objm[k - 1]}_{\frob}\\
        \st                                             & \quad Q(y) = \obj(y), \quad y \in \xpt,
    \end{aligned}
\end{equation}
where~$\mathcal{P}_{n, d}$ is the space of~$n$-variate polynomials of degree at most~$d$ and~$\nabla^2 \objm[-1]$ is set to be zero.
...
They all set~$\fsetm$ in the trust-region subproblem~\cref{eq:trsp} to be~$\Omega$, corresponding the whole space for \gls{newuoa}, a box for \gls{bobyqa} and a polyhedron for \gls{lincoa}.
...
It is cross-platform package providing MATLAB and Python interfaces for using all five Powell's \gls{dfo} solvers, available for Linux, macOS, and Windows at
\begin{center}
...
\begin{matlablst}[%
    ...
    x = pdfo(@fun, x0, Aineq, bineq, Aeq, beq, lb, ub, @nlcon);

    function fx = fun(x)
    ...
    end

    function [cineq, ceq] = nlcon(x)
    ...
    end
\end{matlablst}

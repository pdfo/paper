...
\begin{enumerate}
    ...
    solvers.%, as will be illustrated in~\cref{sec:fake}.
    ...
    %This is explained in~\cref{sec:fake}.
...
line search methods also exist~\cite{Berahas_Byrd_Nocedal_2019,Shi_Etal_2022}.
...
At iteration~$k$, the algorithms construct a linear of quadratic trust-region model~$\objm$ for
...
\begin{equation}
    ...
    \begin{split}
        ...
        \st             & \quad \norm{x - \iter} \le \rad, %\label{eq:trsp-tr}
    \end{split}
\end{equation}
%    \end{align}
%\end{subequations}
where~$\rad$ is the trust-region radius, and~$\norm{\cdot}$ is the~$2$-norm in~$\R^n$.
In this subproblem, the set~$\fsetm \subseteq \R^n$ is a local approximation for the feasible
region~$\fset$. \gls{cobyla} defines~$\fsetm$ by linear interpolants of the constraint
functions over the set~$\xpt$, whereas the other four algorithms take~$\fsetm = \fset$.
...
\begin{equation}
    \label{eq:litpls}
    f_k(y^{\base}) + (y -y^{\base})^\top \nabla f_k(y^{\base})  = f(y),  \quad y \in \xpt,
\end{equation}
...
\alert{After the}
\[
    \xpt[k+1] = \xpt \cup \{\iter^{\tst}\} \setminus\{y_k^{\drop}\}
\]
\[
    \xpt[k+1] = \xpt \setminus\{y_k^{\drop}\} \cup \{\iter^{\geo}\}
\]
...
%interpolation set~$\xpt$ is updated by including the trial point or
$\xpt$ always contains the trust-region center~$\iter$.
...
To do so, at each iteration, it models the objective function with a quadratic of the form%~\eqref{eq:polm},
\begin{equation}
    \label{eq:polm}
    \objm(x) \eqdef \grad^{\T} (x - \iter) + (x - \iter)^{\T} \hess (x - \iter),
\end{equation}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
where~$\ell_{\bar{y}} \colon \R^n \to \R$ denotes the Lagrange function associated with~$\bar{y}$, defined by~$\ell_{\bar{y}}(y) = \delta_{\bar{y}, y}$ for each~$y \in \xpt$, where~$\delta_{\bar{y}, y}$ is the Kronecker delta.

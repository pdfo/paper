...
% 4. Future work: other languages, PRIMA, COBYQA
...
\begin{abstract}
    Late Professor M. J. D. Powell designed five derivative-free optimization methods, namely
    \gls{cobyla}, \gls{uobyqa}, \gls{newuoa}, \gls{bobyqa}, and \gls{lincoa}.
    The algorithms were implemented in Fortran 77 and hence may not be easily accessible to some users.
    This paper introduces the \gls{pdfo} package, which provides user-friendly
    Python and MATLAB interfaces to Powell's code.
    With \gls{pdfo}, users of such languages can call Powell's Fortran solvers easily without any
    need of dealing with the Fortran code.
    ...
    In addition, we provide an overview of Powell's methods, sketching them from a uniform
...
enabling users of such languages to call Powell's solvers without any need to deal with the Fortran code.
...
view point, aiming at easing the understanding of Powell's methods and paving the way for further
development based on them.

The analysis for Powell's \gls{dfo} methods is not within the scope of this paper.
Under some assumptions, Powell's algorithms may be covered by existing theory for trust-region \gls{dfo}
methods~\cite[Chapter~10]{Conn_Scheinberg_Vicente_2009b}. However, it will still be interesting to
pursue a tailored theory for Powell's algorithms.
...
the distance~$norm{y^{\base}-\iter}$ may become unfavorably large for the numerical solution of the
...
\gls{bobyqa} respects the bound constraints~$x\in\fsetm = \fset$ when
...
proper sizes.
Note that we allow one or more of the constraints~\eqref{eq:pdfo-b}--\eqref{eq:pdfo-nl} to be absent.
...
\begin{enumerate}
    ...
    \item Illustrate the potential of \gls{pdfo} in solving hyperparameter optimization problems from
...
Such a unified interface can facilitate the development and comparison of different \gls{dfo} solvers.
The interface of \gls{pdfo} can readily accommodate solvers other than those by Powell, for example,
the COBYQA (Constrained Optimization BY Quadratic Approximations) solver for general nonlinearly
constrained \gls{dfo} problems~(see~\cite[Chapters 5--7]{Ragonneau_2022}
and~\cite{Ragonneau_Zhang_cobyqa}).

Finally, we stress that \gls{pdfo} does not provide Python or MATLAB implementations of Powell's \gls{dfo}
solvers, but only interfaces Powell's implementations with these languages. The implementation of Powell's
solvers in Python, MATLAB, and other languages is a project in progress under the name of
PRIMA~(Reference Implementation for Powell's methods with Modernization and Amelioration)~\cite{Zhang_prima}.

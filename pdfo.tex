...
\begin{enumerate}
    ...
    For example, the user can request \gls{pdfo} to scale the problem according to bound constraints on the variables before solvingt.
\end{enumerate}
...
We choose to test the multiplicative noise because it makes more sense if the scale of the objective
function changes widely, which is often the case in practice.

Given a testing problem, we contaminate its objective function~$\obj$ by a multiplicative Gaussian noise, obtaining
\begin{equation}
    \label{eq:noisy-obj}
...
Here,~$\sigma \ge 0$ is the standard deviation of the noise, representing the noise level.
If we set~$\sigma = 0$, then~$\tilde{f} = f$, corresponding to the noise-free case.

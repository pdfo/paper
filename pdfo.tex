...
\begin{abstract}
    ...
    He also implemented the algorithms into publicly available solvers, which are renowned
    for their efficiency and stability.
    However, these solvers were implemented in Fortran 77 and hence may not be easily accessible to some users.
...
In the underdetermined interpolation, the denominator is lower
...
In addition, \gls{newuoa} maximizes the denominator directly if~\eqref{eq:biglag}
fails to render a large enough denominator, which rarely happens~\cite[\S~6]{Powell_2006}.
...
an~$(n - \rank \aeq)$-dimensional problem by eliminating these constraints.  This is done by a
...
or MATLAB, but only interfaces Powell's implementation with these languages. The implementation of

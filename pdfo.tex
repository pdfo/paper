...
by all solvers during these ten runs.
%by all solvers during these ten runs plus a run with~$\sigma = 0$.
...
The profiles are also averaged over ten independent runs, and, for each problem, the
value~$\obj_{\ast}$ in the convergence test~\eqref{eq:cvt} is set to the least value of~$\obj$ obtained
by all solvers during these ten runs.

...
\def\paragraph{\@startsection{paragraph}{4}{\z@}%
    ...
    {\normalsize\bfseries}}
\makeatother
\def\subclassname{{\bfseries Mathematics Subject Classification}\enspace}
...
\modified{
    ...
    \begin{equation*}
        h = \sgn([{\iter[]}]_i) \sqrt{u} \max \set{\abs{[{\iter[]}]_i}, 1},
    \end{equation*}
    where~$u \approx 2.2 \times 10^{-16}$ is the unit roundoff.
    In our experiment, we also test another value of~$h$, namely
    \begin{equation}
        \label{eq:adaptive-h}
        h = \sqrt{\sigma \max \set{\abs{\obj(\iter[])}, 1}}.
    \end{equation}
...
\modified{
    In the legends, ``\gls{cg}'' and ``\gls{bfgs}'' denote the default methods, whereas ``\gls{cg} adaptive'' and ``\gls{bfgs} adaptive'' are the variations where~$h$ is given by~\eqref{eq:adaptive-h}.

...
\def\paragraph{\@startsection{paragraph}{4}{\z@}%
    ...
    {\sffamily\normalsize\bfseries}}
\makeatother
\def\abstractname{\sffamily Abstract}
\def\keywordname{{\sffamily\bfseries Keywords}}
\def\subclassname{{\sffamily\bfseries Mathematics Subject Classification}\enspace}
\def\ackname{\sffamily Acknowledgements}
...
At iteration~$k$, \modified{they} construct a linear~(for \gls{cobyla}) or quadratic~(for the other methods) model~$\objm$ for the objective function~$f$ according to the interpolation \modified{conditions}
\begin{equation}
    \label{eq:itpls}
...
Powell~\cite{Powell_2013} referred to \modified{this} approach as the \emph{symmetric Broyden update} of quadratic models~(see also~\cite[\S~3.6]{Zhang_2012} and~\cite[\S~2.4.2]{Ragonneau_2022}).
...
\modified{
    ...
    We test two different values of~$h_{\iter[]}$.
...
\modified{In a similar fashion}, we tune the $C$-SVC model detailed in~\cite[\S~2.1]{Chang_Lin_2011} for binary classifications.
...
\modified{Our model is implemented by the \texttt{SVC}} class of the Python package scikit-learn.\footnote{\url{https://scikit-learn.org/stable/modules/generated/sklearn.svm.SVC.html}\,.}
...
We solve~\eqref{eq:hypertune} \modified{using}~\gls{pdfo}, \gls{rs}, \modified{and}~\gls{tpe} to obtain the tuned parameters~$(\bar{C}, \bar{\gamma})$.
    ...
    \modified{
        \begin{tabular}{llS[table-format=1.3]S[table-format=1.3]cS[scientific-notation=true,round-mode=places,table-format=1.2e1]}
            ...
            \cmidrule{2-6}
                                        ...
                                        &                               & 0.997       & 0.977         & 300       & 11366\\
            \cmidrule{2-6}
                                        & \multirow{2}{*}{\gls{tpe}}    & 0.998       & 0.979         & 100       & 3420\\
...
\modified{
    \begin{acknowledgements}
        ...
        Also, we would like to thank Professor Ya-xiang Yuan for his everlasting encouragement and support.
...
The source code \modified{and documentation of \gls{pdfo} are} available at \url{https://www.pdfo.net}\,.

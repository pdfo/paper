...
SciPy provides \gls{cg} and \gls{bfgs} under the \texttt{minimize} in the \texttt{scipy.optimize} module.
...
When~$p = 0.05$, meaning that the function evaluation fails roughly once every $20$ times,
\gls{pdfo} can still solve almost~$60\%$ of the problems up to the convergence
tolerance~$\tau = 10^{-4}$, whereas all its competitors solve less than $25\%$.

...
HSL Mathematical Software Library as subroutine \texttt{VA24}.\footnote{\url{http://www.hsl.rl.ac.uk/.}}
...
\begin{enumerate}
    \item Demonstrate the fact the \gls{pdfo} is capable of adapting to noise without fine-tuning
        ...
        \gls{bfgs} and \gls{cg} on unconstrained CUTEst problems.
    ...
    \item Illustrate the potential of \gls{pdfo} in solving hyperparameter tuning problems from
        ...
        from the Python package~\texttt{hyperopt}.\footnote{
...
The finite-difference \gls{bfgs} and \gls{cg} are provided by SciPy~1.10.0.
...
SciPy provides \gls{bfgs} and \gls{cg} under the \texttt{minimize} in the \texttt{scipy.optimize} module.
...
CUTEst~\cite{Gould_Orban_Toint_2015} problem set using PyCUTEst~\cite{Fowkes_Roberts_Burmen_2022}~1.4.
...
the problems.
Our experiment does not include such fine-tuning, but adopts the default settings of \gls{bfgs} and \gls{cg} in SciPy, as well as the default configurations of~\gls{pdfo}.
...
also include the finite-difference \gls{bfgs} and~\gls{cg} tested before, which do not
handle evaluation failures in any particular way. The solvers are set up in the same way as in the
...
Compared with finite-difference \gls{bfgs} and \gls{cg}, \gls{pdfo} is
...
We speculate that the same approach will also benefit other model-based \gls{dfo} methods,
...
We compare \gls{pdfo} with \gls{tpe} and \gls{rs}, two optimizers for hyperparameter tuning
provided by the Python package~\texttt{hyperopt}.
...
\end{equation}
where~$C\in(0,\infty)$ is a parameter, $z\in\R^{N}$ is the vector whose~$i$th entry is~$z_i$,~$\ones\in\R^N$
is the vector of all ones, and~$Q\in\R^{N\times N}$ is the matrix whose~$(i,j)$ entry is~$K(u_i,u_j)$ for
a kernel function~$K\mathrel{:} \R^{d}\times\R^{d} \to \R$.
We take the Gaussian kernel~$K(u,v) = \exp(-\gamma\|u-v\|^2)$, where~$\gamma \in (0,\infty)$ is another parameter.
See~\cite[\S~2.1]{Chang_Lin_2011} for more details.

As suggested by~\cite[\S~9]{Chang_Lin_2011}, we will tune~$C$ and~$\gamma$ for the performance of
the~$C$-SVC.
We model this process as
\begin{equation}
    \label{eq:hypertune}
...
we define~$P$ based on the AUC score~\cite{Yang_Ying_2022}, which is a number in~$[0,1]$
that measures the quality of a classifier on a dataset, the higher the better.
...
Our experiment is based on binary classifications problems from LIBSVM.\footnote{\url{https://www.csie.ntu.edu.tw/~cjlin/libsvmtools/datasets/}.}
We adopt three datasets detailed in Table~\ref{tab:htdata}.
...
For each dataset~$\mathcal{D}$, we evaluate the function~$P$ based on~$\mathcal{S}$ by the procedure
described above, with
problem~\eqref{eq:svc} being tackled by the SVC class of the Python machine learning package scikit-learn.\footnote{\url{https://scikit-learn.org/stable/modules/generated/sklearn.svm.SVC.html}.}
We solve~\eqref{eq:hypertune} by~\gls{pdfo}, \gls{tpe}, or~\gls{rs} to obtain the tuned
parameters~$(\bar{C}, \bar{\gamma})$.
As in~\cite[\S~5.3]{Ghanbari_Scheinberg_2017}, we modify the feasible set of~\eqref{eq:hypertune}
...
For~\gls{tpe} and~\gls{rs}, we try both~$100$ and~$300$ as the maximal number of function
...
of the tuned parameters, we solve~\eqref{eq:svc} on~$\mathcal{S}$ with~$(C, \gamma)$ set
to~$(\bar{C}, \bar{\gamma})$, and calculate both the AUC score and the accuracy of the resulting
classifier on~$\mathcal{T}$, with accuracy being the fraction of correctly classified data points.
...
%    \begin{tabular}{cS[table-format=3]SSS}
    \begin{tabular}{ccccS}
        \toprule
        Solver      & {AUC Score ($10^{-1}$)}   & {Accuracy ($10^{-1}$)}    &{\#$f$}  & {Time (\SI{}[10^3]{\second})}\\
        \midrule
...
In terms of the AUC score and the accuracy, \gls{pdfo} achieves a clearly better result than \gls{tpe} and \gls{rs} on the ``splice'' dataset, and they all attain comparable results on the other datasets.
...
general hyperparameter tuning problems, which is unlikely true. Instead, we provide an
example to show the possibility of applying Powell's methods to such problems, which is not well studied up to now.
In doing so, we also hope to call for more investigation on \gls{dfo} methods for
machine learning problems in general, echoing the suggestion of~\cite{Ghanbari_Scheinberg_2017}.

...
\begin{abstract}
    ...
    %Alternatively, if the user does not specify any algorithm, the package can also select one
    %automatically based on the problem characteristics.
...
Powell and many other authors used ``direct search'' to mean what is known as ``\glsxtrlong{dfo}'' today. Powell rarely used the word ``derivative-free optimization.''
...
%, and~$\func_k$ is the aforementioned functional.
...
%Note that~$y_k^{\drop}$ is selected after~$\iter^{\trust}$ is obtained.
...
%the approximation~$\fsetm$ of~$\fset$ is set to
...
an~$(n - \rank \aeq)$-dimensional problem.  This is done using a
QR factorization of~$\aeq$. The main motivation for this reduction comes again from~\gls{lincoa},
...
averaging the profiles obtained via the above procedure over ten independent runs. In addition, the

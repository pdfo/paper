...
We stress that \gls{pdfo} does not provide Python or MATLAB implementations of Powell's \gls{dfo}
solvers, but only provide interfaces to Powell's implementations. The implementation of Powell's
...
\gls{pdfo} provides an interface for \gls{dfo} solvers. It can accommodate solvers other than those
by Powell, for example, the COBYQA (Constrained Optimization BY Quadratic Approximation) for general
nonlinearly constrained \gls{dfo} problems~(see~\cite[Chapters 5--7]{Ragonneau_2022}

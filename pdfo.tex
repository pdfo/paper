...
VANDERM4 with some perturbation and randomization. We speculate that other model-based \gls{dfo}
solvers may encounter similar difficulties if they are tested extensively.
...
Repeating such experiments is unnecessary.
Instead, we compare \gls{pdfo} with solvers that
...
%, which will be shown as \mbox{``\gls{pdfo} (no barrier)''} in the results.
...
We now consider a hyperparameter tuning problem from machine learning and compare \gls{pdfo}
...
Similar to~\cite[\S~5.3]{Ghanbari_Scheinberg_2017},
we consider a~$C$-SVC~\cite{Chang_Lin_2011} for binary classification problems with an \gls{rbf}
kernel. We will tune two hyperparameters, namely a regularization parameter~$C > 0$ and a kernel
coefficient~$\gamma > 0$. The objective is to maximize the AUC (area under the ROC curve) of the
classifier.

Our experiments are based on binary classifications problems from the LIBSVM dataset.\footnote{\url{https://www.csie.ntu.edu.tw/~cjlin/libsvmtools/datasets/}.}
A description of the datasets is provided in Table~\ref{tab:htdata}.
...
\begin{table}[ht]
    ...
    \begin{tabular}{ccS[table-format=5]}
        \toprule
...
We use \#$f$ to denote the number of function evaluations, and ``Time'' to represent the computation time.

In terms of \gls{auc} score and accuracy, we observe that \gls{pdfo} achieves a clearly better result than \gls{tpe} and \gls{rs} on the ``splice'' dataset, and they all attain comparable results on all the other datasets.

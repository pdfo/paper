...
% Reviewing
\newcommand{\modified}[1]{\texorpdfstring{{\color{NavyBlue}#1}}{#1}}
...
\modified{
    The methods \gls{cg} and \gls{bfgs} relies on derivatives of the objective function, which are not available in our experiments.
    Therefore, we approximate the derivatives by forward differences, and try two difference parameters:~$h = \sqrt{u} \approx 1.5\times 10^{-8}$, where~$u$ is the unit roundoff, and~$h = \sqrt{\sigma}$, where~$\sigma$ is the noise level of the experiment.
    The former is the default value of the \texttt{scipy.optimize} module, while the latter requires a priori knowledge of the noise level.
}
...
\modified{
    ...
    Note that \gls{pdfo} does not use the value of~$\sigma$ and hence, can be used for solving noisy problems for which the noise level is unknown.
}
...
The performances of \modified{both variations of} \gls{cg} and \gls{bfgs} deteriorate considerably under noise, even if the noise level is not high and the convergence tolerance is not demanding.

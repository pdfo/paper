...
% Redefine the subsection and paragraph headings to be bold.
\makeatletter
...
\institute{%
    T. M. Ragonneau \at
        Department of Applied Mathematics, The Hong Kong Polytechnic University, Hong Kong\modified{, China}\\
...
In addition, the columns of this inverse matrix readily provide the coefficients of Lagrange functions that make the interpolation problem~\eqref{eq:undqitp}--\eqref{eq:leastchange} easy to solve~(see~\cite[\S~3]{Powell_2004b})
...
\modified{
    These methods rely on gradient of the objective function, which is not available in our experiments.
    Therefore, we approximate it by forward differences, testing two difference parameters~$h$.
...
\modified{
    In the legends, ``\gls{cg}'' and ``\gls{bfgs}'' denote the finite-difference methods with~$h = \sqrt{u}$, and ``\gls{cg} adaptive'' and ``\gls{bfgs} adaptive'' are the variations with~$h = \sqrt{\sigma}$.
    Let us stress again that \gls{pdfo} does not require the value of~$\sigma$ and can be used for solving noisy problems for which the noise level is unknown.
}
...
The performances of \gls{cg} and \gls{bfgs} \modified{with~$h = \sqrt{u}$} deteriorate considerably under noise, even if the noise level is not high and the convergence tolerance is not demanding.
\modified{
    As expected, the adaptive difference parameter~$h = \sqrt{\sigma}$ improves the performance of \gls{cg} and \gls{bfgs} by a large margin, but they are still inferior to \gls{pdfo}.
    This shows the performance of \gls{pdfo} in noisy settings, even when compared to methods that explicitely use a priori knowledge about the noise level.
}
We do not include results with larger values of~$\sigma$, as \gls{cg} and \gls{bfgs} \modified{with~$h = \sqrt{u}$} will barely solve any problem, while \gls{pdfo} can still tackle a significant proportion of them up to a reasonable precision\modified{, and continues to outperform \gls{cg} and \gls{bfgs} with~$h = \sqrt{\sigma}$}.

\modified{
    The fact that \gls{cg} and \gls{bfgs} with~$h = \sqrt{u}$ have unfavorable performance is not surprising.
    As mentioned, for example, by~\cite{Shi_Etal_2022a,Shi_Etal_2023}, the difference parameter must be chosen carefully according to the noise level and the smoothness of the problems.
    In the above experiment, we used the only knowledge we had: the noise level.
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Performance profiles of \gls{cg}, \gls{bfgs}, \gls{pdfo}, and~\gls{pdfo} without barrier on unconstrained problems with the objective functions evaluated by~$\hat{\obj}_p$ in~\eqref{eq:nan-obj}}
    \label{fig:nan}
\end{figure}
...
When~$p = 0.05$, the function evaluation fails roughly once every $20$ times, but \gls{pdfo} can still solve almost~$60\%$ of the problems up to the convergence tolerance~$\tau = 10^{-4}$ in the sense of~\eqref{eq:cvt}, whereas all its competitors solve less than $25\%$.

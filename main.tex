...
the code publicly available. The solvers constitute the cornerstones of~\gls{pdfo}.
In the sequel, we provide a brief overview on these five algorithms, starting with a sketch in
Subsection~\ref{ssec:sketch} and then presenting more details in the subsections after.

\subsection{A sketch of the algorithms}
\label{ssec:sketch}

Powell's model-based \gls{dfo} algorithms are trust-region methods.
%, and the trust-region models are obtained by linear or quadratic interpolation.
At iteration~$k$, the algorithms construct a linear of quadratic trust-region model~$\objmdl$ for
the objective function~$f$ to meet the interpolation condition
\begin{equation}
    \label{eq:itpls}
    \objmdl(y) = \obj(y), \quad y \in \itpls,
\end{equation}
where~$\itpls \subseteq \R^n$ is a finite interpolation set  updated along the iterations.
\gls{cobyla} models the constraints by interpolants on~$\itpls$ as well.
Instead of repeating Powell's description of these algorithms,
we provide a sketch of them in the sequel, emphasizing the trust-region subproblem, the
interpolation problem, and the interpolation~set.%


\subsubsection{The trust-region subproblem}
In all the five algorithms, iteration~$k$ places the trust-region center~$\iter$ at
the ``best'' point where the objective function and constraints have been evaluated
so far. Such a point is selected according to the objective function or a merit
function that takes the constraints into account.
After choosing the trust-region center~$x_k$, with the trust-region model~$f_k$ constructed
according to~\eqref{eq:itpls}, a trial point is then obtained by solving approximately
...
%    \end{align}
%\end{subequations}
where~$\trg$ is the trust-region radius, and~$\norm{\cdot}$ is the~$2$-norm in~$\R^n$.
In this subproblem, the set~$\srchspmdl \subseteq \R^n$ is a local approximation for the feasible
region~$\srchsp$. \gls{cobyla} defines~$\srchspmdl$ by linear interpolants of the constraint
functions over the set~$\itpls$, whereas the other four algorithms take~$\srchspmdl = \srchsp$.
The trust-region subproblem solvers for~\eqref{eq:trsp} will be introduced in Subsections~\ref{}--\ref{eq:}.


\subsubsection{The interpolation problem}
The interpolation condition~\eqref{eq:itpls}  is essentially a linear system with respect to the
coefficients of the model.
Given a base point~$y^{\base}\in \R^n$, which may depend on~$k$, a linear model~$f_k$ takes the form
of~$f_k(x) = f(y^{\base}) + (x-y^{\base})^\top \nabla f_k(y^{\base})$,
and hence~\eqref{eq:itpls} is equivalent to
\begin{equation}
    \label{eq:litpls}
    f_k(y^{\base}) + (y -y^{\base})^\top \nabla f_k(y^{\base})  = f(y),  \quad y \in \itpls,
\end{equation}
which is a linear system with~$f(y^\base) \in \R$ and~$\nabla f(y^\base) \in \R^n$ being the
unknowns, the degrees of freedom being~$n+1$. \gls{cobyla} builds linear models by the
system~\eqref{eq:litpls}.

Similarly, if~$f_k$ is a quadratic model, then~\eqref{eq:itpls} is equivalent to
\begin{equation}
    \label{eq:qitpls}
    f_k(y^{\base}) + (y -y^{\base})^\top \nabla f_k(y^{\base})
    + \frac{1}{2}(y-y^{\base})^\top  \nabla^2 f_k(y^{\base}) (y-y^{\base}) = f(y),  \quad y \in \itpls,
\end{equation}
...
is necessary that~$\itpls$ contains~$(n+2)(n+1)/2$ points, and~$f$ should have been evaluated at all
...
interpolation condition~\eqref{eq:itpls} with~$|\itpls|$ less than~$(n+2)(n+1)/2$, the
...
the interpolation set in order to guarantee the well conditioning of the interpolation problem.
\red{After the}
\[
    \itpls[k+1] = \itpls \cup \{\iter^{\tst}\} \setminus\{y_k^{\drop}\}
\]
\[
    \itpls[k+1] = \itpls \setminus\{y_k^{\drop}\} \cup \{\iter^{\geo}\}
\]
...
$\itpls$ always contains the trust-region center~$\iter$.
...
%At the dawn of the modern \gls{dfo},
Powell released the \gls{cobyla} solver in May 1992 and published the \gls{cobyla}
paper\cite{Powell_1994} in~\citedate{Powell_1994}. The solver is named after \enquote{\glsxtrlong{cobyla}}.
It aims to solve the optimization problem~\eqref{eq:nlc} with the feasible region~$\srchsp$
being
\[
    \srchsp \defeq \set{x \in \R^n \colon \cgen(x) \ge 0,\; i=1,...,m},
\]
where~$\cgen \colon \R^n \to \R$ denotes the~$i$th constraint function ($i \in \set{1, 2, \dots, m}$).
...
interpolants on the interpolation set~$\itpls$, which consists of~$n+1$ points that are updated
along the iterations.
In this context, the interpolation set~$\itpls$ is \red{poised} if an only if
the volume of the~$n$-simplex engendered by~$\itpls$ to be nonzero.

Once the linear models~$\cgenmdl$ of the constraint functions~$\cgen$ for~$i \in \set{1, 2, \dots, m}$
are built, the approximation~$\srchspmdl$ of~$\srchsp$ is set to
\begin{equation}
    \label{eq:cobylarg}
...
Solving sequentially the problems~\eqref{eq:trsp} by replacing the trust-region radius~$\trg$ with
a constant continuously increasing from zero to~$\trg$ will generate a piecewise linear
...
However, the trust-region constraint~$\|x-x_k\|\le \trg$
...
\begin{align*}
    \min_{x \in \R^n}   & \quad \max_{1\le i \le m} [\cgenmdl(x)]_{-}\\
    \st         & \quad \norm{x - \iter} \le \trg,
\end{align*}
...
\begin{equation}
    ...
    \begin{aligned}
        ...
        \objmdl[k - 1]}_{\fro}\\
        \st                                 & \quad Q(y) = \obj(y), \quad y \in \itpls,
    \end{aligned}
\end{equation}
...
\begin{subequations}
    ...
    \begin{align}
        ...
        \st        & \quad \bl \le x \le \bu, \label{eq:pdfo-b}\\
...
\begin{figure}[ht]
    \begin{subfigure}{.48\textwidth}
        \centering
        %\insertprofiles{{"newuoa","bobyqa","lincoa","cobyla","uobyqa"}}{figures/perf-plain-10.csv}{4}
